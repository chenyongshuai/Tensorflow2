...
class Layer:
    #全连接网络层的初始化方法
    def __init__(self, n_intput, n_neurons, activition=None, weights=None, bias=None):
        """
        :param n_intput: 输入节点个数
        ...
        np.sqrt:计算数组各元素的平方根
        """
        self.weights= weights if weights is not None else np.random.randn(n_intput,n_neurons) * np.sqrt(1/n_neurons)
    ...
    def activate(self,x):
        ...
        self.last_activition = self._apple_activation(r) #获取全连接层的输出
