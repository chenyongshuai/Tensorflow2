...
def draw(weight):
    ...
    features = random.randint(-10,10,(100,2))
    result=sigmoid(features.dot(weight[1:3,:])+weight[0:1,:])
    ...
    for i in range (0,features.__len__()):
        if result[i,:][0,0]>0.5:
            x0List.append(float(features[i,:][0]))
            y0List.append(float(features[i,:][1]))
        else:
            x1List.append(float(features[i, :][0]))
            y1List.append(float(features[i, :][1]))
